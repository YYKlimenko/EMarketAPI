...
from market.schemas import CreatingUser
...
async def get_user(_id: int, service: UserService = Depends()):
...
async def delete_user(_id: int, service: UserService = Depends()):
    return await service.delete(_id)


@router.post('/users/registration/', status_code=201, description='Create the new user')
async def registrate_user(user: CreatingUser, service: UserService = Depends()):
...
async def change_user_data(data: dict, _id: int, service: UserService = Depends()) -> None:
